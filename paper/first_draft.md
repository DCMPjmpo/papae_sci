...
joins this cluster only at a substantially higher dendrogram height
(≈ 0.50), a separation that is also reflected in the pairwise distance
matrix: the minimum PABP-to-other distance is 0.389 (PABP–TEM1),
comparable in magnitude to the maximum within-other-four distance of
0.397 (CBS–GAL4). Across the heatmap, dendrogram and PCA analyses,
PABP remains the most isolated of the five proteins.
...
LLR alone. SCI and LLR were only moderately correlated at the
pooled level (Pearson r = 0.456; Spearman ρ = 0.467;
`sci_llr_correlation.csv`), indicating partial overlap but
substantial non-shared variance. At the per-protein level LLR exceeds SCI on every one of
...
| GAL4 | 6.29  | n/a | 5.18  | 4.59  | (see CSV) |
| PABP | 56.55 | (T_obs=1; see Methods §4.6 note) | 38.71 | 42.80 | (see CSV) |
| PTEN | 2.88  | n/a | 14.57 | 17.05 | (see CSV) |
| TEM1 | 23.59 | n/a | 15.19 | 15.94 | (see CSV) |
...
**Isolation of PABP.** minimum PABP-to-other = 0.389
(PABP–TEM1); maximum within-other-four = 0.397 (CBS–GAL4); minimum
within-other-four = 0.115 (PTEN–TEM1). PABP remains the most
isolated protein in the clustering analysis, joins the remaining
proteins only at a substantially higher dendrogram height, and the
PCA and distance analyses consistently place PABP apart from the
other four proteins.
...
| Sup Table S2 | Full per-statistic per-scope results (z, Cohen's *d*, p, q, CIs, bootstrap) | Results §2.1–2.7; Methods §4.7–§4.10 |
...
Cited in this draft (alphabetical, surname-first):

- Belrose et al., 2023 — tuned-lens decoding.
- Brandes et al., 2023 — proteome-scale ESM-1b variant scoring.
- Deng et al., 2012 — TEM1 DMS source dataset.
- Detlefsen et al., 2022 — task-dependent best single layer in PLMs.
- Elnaggar et al., 2022 — ProtTrans / per-layer probing.
- Firnberg et al., 2014 — TEM1 DMS source dataset.
- Frazer et al., 2021 — EVE; variant effect from sequence.
- Geva et al., 2021 — feed-forward layers as key–value memories.
- Hayes et al., 2025 — ESM3.
- Hsu et al., 2022 — ridge regression over PLM embeddings.
- Jacquier et al., 2013 — TEM1 DMS source dataset.
- Kitzman et al., 2015 — GAL4 DMS source dataset.
- Lin et al., 2023 — ESM-2 (esm2_t33_650M_UR50D).
- Marquet et al., 2022 — variant-of-uncertain-significance scoring.
- Matreyek et al., 2021 — PTEN DMS source dataset.
- Meier et al., 2021 — ESM-1v masked-marginal LLR baseline.
- Melamed et al., 2013 — PABP DMS source dataset.
- Mighell et al., 2018 — PTEN DMS source dataset.
- Notin et al., 2022 — Tranception.
- Notin et al., 2023 — ProteinGym.
- Phipson & Smyth, 2010 — empirical p-value convention.
- Rao et al., 2019 — TAPE; PLM probing benchmark.
- Rao et al., 2021 — MSA Transformer / per-head attention; ICLR
  contact-prediction pipeline pooling attention across all 33 ESM
  layers.
- Rives et al., 2021 — ESM-1b.
- Simon & Zou, 2025 — per-layer sparse-autoencoder analysis of PLMs.
- Stiffler et al., 2015 — TEM1 DMS source dataset.
- Sun et al., 2020 — CBS DMS source dataset.
- Tenney et al., 2019 — scalar-mix probing of BERT.
- Vig et al., 2021 — per-head attention analysis of PLMs.
