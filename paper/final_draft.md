# Final Draft — Manuscript (assembled)

> Single-file assembly of the per-section draft files under `paper/`.
> Section order follows the Nature Communications convention:
> Title → Abstract → Introduction → Results → Discussion → Methods →
> Figure Legends → Supplementary Information → References.
>
> No textual content has been rewritten. Section numbering is unified
> across the assembled file (Results §2.1–§2.8; Discussion D1–D5;
> Methods §4.1–§4.12). Source files retained verbatim:
> `paper/introduction.md`, `paper/results.md`,
> `paper/discussion_full.md`, `paper/methods.md`,
> `paper/figure_map.md`, `paper/supplementary.md`. Title and Abstract
> are reused verbatim from `paper/manuscript_framework.md` (the
> per-section drafts do not contain a Title or Abstract block).
...
13 of these 15 tests (T₁, T₃, T₃ˢᵗᵈ across 5 proteins) retained
...
respectively). The signal is therefore
not pooled out of one over-represented protein; it is reproduced
within each, with strongest consistency for T₃ and T₃ˢᵗᵈ across
all five proteins.
...
Bonferroni significance at α = 0.05 was retained for T₃ and T₃ˢᵗᵈ in
all five proteins, and for T₁ in three of five proteins (GAL4, PABP, TEM1).
CBS-T₁ (p_bonf = 0.60) and PTEN-T₁ (p_bonf = 0.14) did not survive Bonferroni.
...
K = 528, multiple sanity-check resamples in
...
reported in this study, including but not limited to:
...
preprocessing pipeline. The full permutation + bootstrap pipeline runs
in ≈ 8–10 min single-threaded on a CPU at the real-scale (95,142
mutations, 528 layer pairs, 1,000 adaptive permutations + 1,000
bootstrap resamples), with peak resident memory ≈ 410 MB.
